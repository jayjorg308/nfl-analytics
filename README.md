...
[![Vercel Deploy](https://vercelbadge.vercel.app/api/jayjorg308/nfl-analytics)](https://vercel.com/jayjorg308/nfl-analytics)
...
repo). The database is a **Neon** serverless Postgres branch; production and dev
are separate Neon branches, and ingestion writes are validated by diffing a
prod verifier run against dev's known-good output
([`scripts/verify-phase3a.mjs`](scripts/verify-phase3a.mjs)).

> **Deploy badge note:** Vercel has no official deploy-status badge. The badge
> above uses the community [`vercelbadge.vercel.app`](https://github.com/Sun-ZhenXing/vercel-badge)
> service — update the `jayjorg308/nfl-analytics` path to your actual
> **Vercel account slug / project name** if it differs from the GitHub path.
...
Building toward a **v1 ship** along [ADR-0010](docs/adr/0010-v1-build-sequence.md)'s
slice sequence.
...
1. **MOV-ELO methodology investigation** (`/research/elo-methodology`) — the first
   ...
   before publishing — [ADR-0022 §2](docs/adr/0022-elo-application-notes-and-tie-correction.md).)
...
eight tabs that I tab-switched through every week to prep picks and game-watching
notes for me and my friends. The spreadsheet got the job done, but it had a
ceiling: the data was hand-entered and shallow, the formulas were fragile, splits
were copy-paste exercises, and any question I couldn't squeeze into the existing
columns simply went unanswered.
...
questions I'm curious about — "is last-3-games production actually predictive of
the next game, or does it just feel that way?" — as proper, written-up
investigations instead of gut feel.

It's also a deliberate showcase of how I think about data engineering and
product design: schema-first, decision-driven (every non-trivial call is captured
as an ADR), and shipped in vertical slices so integration risk surfaces early.
