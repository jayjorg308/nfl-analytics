...
try {
  ...
  const c = (
    ...
    SELECT (SELECT count(*) FROM season) seasons,
           ...
           (SELECT count(*) FROM team_week_stats WHERE week >= 19) playoff`)
  ).rows[0];
  ...
  console.log(
    `  game rows = ${c.games} (data-dependent; 1424 with the cancelled 2022 BUF-CIN game)`
  );
  ...
  const shape = (
    ...
    GROUP BY s.year, tws.week ORDER BY s.year, tws.week`)
  ).rows;
  const byYear = {};
  ...
  for (const year of Object.keys(byYear).sort()) {
    const w = byYear[year];
    console.log(
      `    ${year}: ${[0, 1, 18, 19, 20, 21, 22].map((k) => `${k}=${w[k] ?? 0}`).join(' ')}`
    );
  }
  ...
  check(
    '2026 wk0 = 32 and no later weeks',
    byYear[2026]?.[0] === 32 && !byYear[2026]?.[1]
  );
  ...
  const agg = (
    ...
    FROM team_week_stats tws JOIN season s ON s.id = tws.season_id
    WHERE s.year = 2026 AND tws.week = 0`)
  ).rows[0];
  check('2026 baseline = 32 teams', Number(agg.n) === 32, `got ${agg.n}`);
  check(
    '2026 baseline mean = 1500.0 (conserved total)',
    Math.abs(Number(agg.mean) - 1500) < 0.1,
    `got ${agg.mean}`
  );
  ...
  const base = (
    ...
    WHERE s.year = 2026 AND tws.week = 0 ORDER BY t.abbreviation`)
  ).rows;
  console.log('  all 32 sorted by abbr (the diffable baseline):');
  for (const r of base)
    console.log(
      `    ${r.a.padEnd(3)} elo=${String(r.elo).padStart(7)} sos=${r.sos}`
    );
  ...
  const wk0 = (
    ...
    FROM team_week_stats tws JOIN team t ON t.id = tws.team_id JOIN season s ON s.id = tws.season_id
    WHERE t.abbreviation = 'KC' AND tws.week = 0 ORDER BY s.year`)
  ).rows;
  const endr = (
    await pool.query(`
    SELECT DISTINCT ON (s.year) s.year, ROUND(tws.elo_rating::numeric, 1) elo
    FROM team_week_stats tws JOIN team t ON t.id = tws.team_id JOIN season s ON s.id = tws.season_id
    WHERE t.abbreviation = 'KC' ORDER BY s.year, tws.week DESC`)
  ).rows;
  const endMap = Object.fromEntries(endr.map((r) => [r.year, r.elo]));
  for (const r of wk0)
    console.log(
      `    ${r.year}: wk0=${String(r.elo).padStart(7)}  season-end=${String(endMap[r.year]).padStart(7)}`
    );
  ...
  const o = (
    ...
    SELECT (SELECT count(*) FROM team_week_stats x LEFT JOIN season s ON s.id = x.season_id WHERE s.id IS NULL) tws_s,
           ...
           (SELECT count(*) FROM game x LEFT JOIN team t ON t.id = x.away_team_id WHERE t.id IS NULL) g_a`)
  ).rows[0];
  check(
    'no orphans',
    [o.tws_s, o.tws_t, o.g_s, o.g_h, o.g_a].every((v) => Number(v) === 0),
  ...
  async function spot(label, where, expect) {
    const r = (
      ...
      WHERE ${where} ORDER BY g.nflverse_game_id LIMIT 1`)
    ).rows[0];
    if (!r) return check(`${label} present`, false);
    const typesOk =
      r.dt instanceof Date &&
      typeof r.neutral === 'boolean' &&
      typeof r.intl === 'boolean' &&
      typeof r.gt === 'string';
    const gtOk =
      expect.gt === null
        ? ['wildcard', 'divisional', 'conference'].includes(r.gt)
        : r.gt === expect.gt;
    const valsOk =
      gtOk && r.neutral === expect.neutral && r.intl === expect.intl;
    console.log(
      `  [${label}] ${r.id}: ${r.away}@${r.home} ${r.asc_}-${r.hs}  dt=${r.dt.toISOString()}  type=${r.gt} neutral=${r.neutral} intl=${r.intl}`
    );
    check(`${label}: types are Date/bool/bool/enum`, typesOk);
    check(
      `${label}: values`,
      valsOk,
      `expected gt=${expect.gt ?? 'playoff'} neutral=${expect.neutral} intl=${expect.intl}`
    );
  }
  await spot('super-bowl/neutral', "g.game_type = 'super_bowl'", {
    gt: 'super_bowl',
    neutral: true,
    intl: false,
  });
  await spot('international', 'g.is_international', {
    gt: 'regular',
    neutral: true,
    intl: true,
  });
  await spot(
    'playoff/non-neutral',
    "g.game_type IN ('wildcard','divisional','conference') AND NOT g.is_neutral_site",
    { gt: null, neutral: false, intl: false }
  );
  await spot(
    'regular/non-neutral',
    "g.game_type = 'regular' AND NOT g.is_neutral_site",
    { gt: 'regular', neutral: false, intl: false }
  );
  ...
  console.log(
    "  Hand-verify >=3 games' MOV-ELO against the formula. Per the ADR-0014 rulings, GROW"
  );
  console.log(
    '  the set to include +1 tie game (mult=1) and +1 non-neutral playoff game (K=20 +'
  );
  console.log(
    '  HFA=50) — the cold-start/mid-chain/neutral-SB trio does not cover those cases.'
  );
